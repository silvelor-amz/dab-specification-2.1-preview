...
Version: 2.1  |  Last Updated: 2024-04-09

## 2.1 Release Notes

### New Operations
- Added power mode operations:
  - `system/power-mode/get`: Get the current power mode of the device
  - `system/power-mode/set`: Set the device power mode (Active/Standby)
- Added application management operations:
  - `applications/install`: Install an application from a URL
  - `applications/uninstall`: Remove an application from the device
  - `applications/clear-data`: Clear all application data
  - `applications/install-from-app-store`: Install or update an application from the device's app store
- Added system operations:
  - `system/factory-reset`: Perform a full factory reset
  - `system/network-reset`: Reset all network settings
  - `system/logs/start-collection`: Begin collecting system logs
  - `system/logs/stop-collection`: Stop collecting logs and return them
  - `system/setup/skip`: Skip device setup wizard after factory reset
- Added content operations:
  - `content/search`: Search for content available on the device
  - `content/recommendations`: Retrieve content recommendations
  - `content/open`: Open specified content
- Added new system settings:
  - `brightness`: Control screen brightness level
  - `contrast`: Control screen contrast level
  - `timeZone`: Set device time zone using IANA formats
  - `screenSaver`: Enable/disable screensaver
  - `screenSaverTimeout`: Set screensaver timeout in seconds
  - `screenSaverMinTimeout`: Minimum screensaver timeout value
  - `personalizedAds`: Enable/disable personalized advertisements
  - `highContrastText`: Enable/disable high contrast text
  - `identifierForAdvertising`: Control advertising identifier

### Enhancements
- Added requirements section (3.1-3.6) with miscellaneous compliance criteria
- Added operation latency requirements
...
- 3.5 Regardless of whether the implementation is Native or Bridge, operations MUST be completed within 200ms, unless otherwise noted in the DAB Operations documentation in section 5 of this spec.
...
|Time Zone|`TimeZone`|All time zones are strings using the IANA formats defined in [tzdb](https://www.iana.org/time-zones)|"Europe/Helsinki"|
...
```json
{
   ...
   "operations": [
      ...
      "applications/clear-data",
      "applications/install-from-app-store",
      "device/info",
      "system/power-mode/get",
      "system/power-mode/set",
      "system/restart",
...
### Operation: Install the application
*Operation model: Request / Response*

This operation will sideload the application provided by the request url field. If the application is already installed, applications/install will first uninstall it and then install the application package from url.

#### Request Topic

`dab/<device-id>/applications/install`

#### Operation Latency

The operation MUST be completed within 60 seconds.

#### Request Format

```typescript
interface InstallApplicationRequest extends DabRequest {
   appId: string;
   url: string;
   format?: string;
   timeout?: number;
...
--- | --- | --- 
appId | - | Application Identifier.
url | - | Application package url.
format | - | Optional application package format. This only applies if there are multiple different formats for an installation.  
timeout | - | Optional action timeout parameter in seconds. If the installation takes longer than the timeout, your implementation must stop the installation process and return a 408 timeout response.

#### Sample Request
```typescript
{
   "appId": "YouTube",
   "url": "http://192.168.2.108/app/youtube",
   "format": "apk"
}
```
...
```typescript
type InstallApplicationResponse = DabResponse
```

### Operation: Uninstall the application
*Operation model: Request / Response*

This operation uninstall the application from the device. Note, this should also delete any local storage. 
If the application cannot be uninstalled, the device should respond with a 403 response.

#### Request Topic

`dab/<device-id>/applications/uninstall`
...
```typescript
interface UninstallApplicationRequest extends DabRequest {
   appId: string;
}
```
...
```typescript
type UninstallApplicationResponse = DabResponse
```

### Operation: Clear the application data
*Operation model: Request / Response*

This operation will clear all the application data on the device. 

#### Request Topic

`dab/<device-id>/applications/clear-data`

#### Request Format

```typescript
interface ClearApplicationDataRequest extends DabRequest {
   appId: string;
}
```
...
```typescript
type ClearApplicationdataResponse = DabResponse
```

### Operation: Install the application from App Store
*Operation model: Request / Response*

This operation will install the application to the latest version available on the device app store. If the application is already installed, this operation will just update the app to the latest version from App store. If the app store requires a user login, the device should respond with a 401 unauthorized error code.


#### Request Topic

`dab/<device-id>/applications/install-from-app-store`
...
```typescript
interface InstallFromAppStoreRequest extends DabRequest {
   appId: string;
}
```
...
```typescript
type InstallFromAppStoreResponse = DabResponse
```
...
### Operation: Get the device power mode
*Operation model: Request / Response*
...
Standby| The device is in a low-power state, ready to resume operation quickly.

#### Request Topic

`dab/<device-id>/system/power-mode/get`
...
#### Response Format
```typescript
interface GetPowerModeResponse extends DabResponse {
   powerMode: PowerMode;
}
```
...
Parameter | Description
--- | ---
powerMode | The current power mode of the device.
...
### Operation: Set the device power mode
*Operation model: Request / Response*

Sets the power mode of the device.
The operation returns a response after the device has been switched to the specified power mode.
DAB must remain active regardless of the power mode.
If the device is already in the requested power mode, its status is maintained, and a response is returned.
Upon completion, the operation returns the device's current power mode.

#### Request Topic

`dab/<device-id>/system/power-mode/set`

#### Request Format

```typescript
interface SetPowerModeRequest extends DabRequest {
   powerMode: PowerMode;
}
```
...
Parameter | Default|  Description
--- | --- | ---
powerMode | - | The power mode to which the device is set.
...
#### Response Format
```typescript
interface SetPowerModeResponse extends DabResponse {
   powerMode: PowerMode;
}
```
...
This operation will start the device restart sequence. After the device is restarted, DAB should stay enabled; DAB remaining enabled is the only expectation of the specification. The response should be sent after this operation is initiated.
...
This operation must invoke a full factory reset -- similar to the UI menu option that users can select. DAB response must be provided prior to commencing the factory reset. DAB must remain enabled after factory reset. The DAB Device ID must not change after the reset. It is recognized that loss of network (ie. WiFi) may require users to intervene after factory reset. The response should be sent after this operation is initiated.

#### Request Topic

`dab/<device-id>/system/factory-reset`

#### Request Format

```typescript
type FactoryResetDeviceRequest = DabRequest
```

#### Response Format
```typescript
type FactoryResetDeviceResponse = DabResponse
```
...
{
   "status": 500,
   "error": "The device ran into an internal issue while performing the reset. Aborting."
}
```
...
This operation resets all network settings on the device. It must remove all stored network configurations (such as Wi-Fi credentials, encryption keys, and DHCP leases) and revert network settings to their defaults. This ensures the device reestablishes network connections from scratch, re-resolving anything tied to previous configurations. DAB must remain functional after the reset. The response should be sent after this operation is initiated.

#### Request Topic
`dab/<device-id>/system/network-reset`
...
#### Response Format
```typescript
type NetworkResetResponse = DabResponse
```
...
{
    "status": 500,
    "error": "Failed to restart network module"
}
```
...
brightness              | Brightness level adjustment range
contrast                | Contrast level adjustment range
...
audioVolume             | Audio output volume control range
mute                    | Volume mute operation support on the device
textToSpeech            | Text to Speech support on the device
screenSaver             | Screensaver support on the device
screenSaverMinTimeout   | Minimum screensaver timeout values in seconds supported by the device. The value should be as low as possible and in any case not greater than 60 seconds.
personalizedAds         | Personalized Ads support on the device
highContrastText        | High Contrast Text Setting support 
identifierForAdvertising| Personalized Ads identifier

For settings with boolean values:
- true: the feature is supported
- false: the feature is not supported

For settings with min/max values:
- min: the minimum supported value for the setting
- max: the maximum supported value for the setting
...
contrast                | Current contrast level
...
screenSaver             | Current screensaver setting state
screenSaverTimeout      | Current screensaver timeout setting in number of seconds
personalizedAds         | Current personalized Ads state (disabled = the user opted out)
highContrastText        | Current High Contrast Text state
identifierForAdvertising| Current identifier for advertising string
...
This operation is used to update a discrete settings on the device.
...
### Operation: Skip Device Setup Wizard
*Operation model: Request / Response*

This operation is called when the device is in the setup screen after a factory-reset. When called, devices must bypass the setup wizard and land to the home-page. It is understood that account specific functionality like the application store, recommendations, and search may not function without additional manual user account log-in.

#### Request Topic
`dab/<device-id>/system/setup/skip`
...
#### Request Format
```typescript
type SkipSetupWizardRequest = DabRequest
```

#### Response Format
```typescript
type SkipSetupWizardResponse = DabResponse
...
{
   "status": 400,
   "error": "Unexpected request when setup wizard is not currently active."
}
```

### Operation: Start Collecting System Logs
*Operation model: Request / Response*

This operation indicates the device to start accumulating logs from runnning system and user applications. 

The following log categories are expected to be stored:
1. System Logs - Logs emitted from system services (i.e A/V Decoder, Power Manager, Networking Module)
2. Application Logs - Logs emitted from apps (i.e.  Prime Video, YT, Netflix)
3. Crash Logs - Dumps and logs generated when either system services or applications crash

Logs must be returned when log collection is completed with `system/logs/stop-collection`, described below.

#### Request Topic

`dab/<device-id>/system/logs/start-collection`

#### Request Format

```typescript
interface StartSystemLogCollectionRequest extends DabRequest {
}
```
...
```typescript
interface StartSystemLogCollectionResponse extends DabResponse
...
{
   "status": 500,
   "error": "No more disk space, cannot begin system log collection."
}
```

### Operation: Stop Collecting System Logs
*Operation model: Request / Response*

This operation indicates the device to stop accumulating logs and respond with the collected logs so far.

Devices must package the following categories of logs into a tar.gz file with the following folder structure.
...
📦logs.tar.gz
 ...
 ┃ ┗ 📂dab_app_id
 ┣ 📂crash
 ┃ ┣ 📜 dump_av_trace.vmcore
 ┃ ┗ 📜...
```

The intention of the structure is defined below: 

* `system` - Contains relevant system logs generated by internal devices services. Ex: AV Pipeline, App Manager, etc...
* `application` - Contains logs produced by applications, bucketized into folders titled the DAB Application Identifier. For specific log files to collect per application, refer to the private DAB Confluence Page shared with device and app partners.  
* `crash` - Contains files believes to be generated as a result of a crash of a system service or application. May be empty if no crash occured. 

The tar.gz must then be base64 encoded and included in the response as per the specification below. Only logs generated between the start and stop collection requests must be returned.

When the `logArchive` exceeds the MQTT broker's message size limit (e.g., 200 MB) or the device platform's memory constraints, the response must be split into chunks. Each chunk will contain a portion of the base64-encoded `logArchive` and will be sent in multiple MQTT responses. The `remainingChunks` parameter will indicate how many more chunks are expected. The responses must be sent in order, and DAB clients will wait until all chunks are received, which occurs when `remainingChunks` is set to 0.

#### Request Topic

`dab/<device-id>/system/logs/stop-collection`

#### Operation Latency

Max period between chunk response should be at most 2 seconds.

#### Request Format

```typescript
interface StopSystemLogCollectionRequest extends DabRequest
```

#### Response Format

```typescript
interface StopSystemLogCollectionResponse extends DabResponse {
    logArchive: string // Byte64 encoded string from assembled tar.gz  containing logs with aforementioned folder structure 
    remainingChunks: number // Remaining StopSystemLogCollectionResponse that are expected to form the complete logArchive 
}
```
...
The operation MUST be completed within 200 milliseconds plus the duration of the keypress. 
...
This operation captures the current output of the device and returns an image that is base64 encoded. The image must include both the video and graphics plane content. The image data must be in PNG format.
...
```typescript
type CaptureScreenshotRequest = DabRequest
```
...
#### Operation Latency
The operation MUST be completed within 700 milliseconds.
...
The following operations allow retrieval and consumption of content (e.g., movies, TV shows) available on the device.

Content is associated with an application that can open it.
If the same content can be opened by multiple applications, there will be a separate `ContentEntry` for each application, each with its own unique entry ID.

The operations must not change what is displayed on the screen.

```typescript
interface ContentEntry {
   // A unique identifier of the content.
   entryId: string,
   // The identifier of the application that opens the content.
   appId: string,
   // The title of the content. The language of the title should match the device's settings.
   title: string,
   // Base64-encoded PNG image of the content's poster in a data URL format.
   poster: string,
   // The list of category of the content.
...
enum ContentCategory {
   ...
   LinearTv = "LinearTv",
   Video = "Video", // as social media and online video
   Trending = "Trending",
   Others = "Others",
}
```
...
This operation searches for content available on the device.
The operation mimics a user performing a search in the device's search menu.

#### Request Topic

`dab/<device-id>/content/search`

#### Request Format

```typescript
interface SearchContentRequest extends DabRequest {
   searchText: string;
}
```
...
Parameter | Default|  Description
--- | --- | ---
searchText | N/A | The string to be used for the search.
...
```typescript
interface SearchContentResponse extends DabResponse {
...
```json
{
   ...
   "entries": [
      {
         "entryId": "65850a53-3cf1-4b0e-bc02-d015fa9bec77",
         "title": "Home Alone",
         "appId": "PrimeVideo",
         "poster":"data:image/png;base64, iVBsRw0KdGgoAAAANSUhE...",
         "categories": ["Movies"],
      },
      {
         "entryId": "b8884dfa-60e8-448a-99f3-bcf283dcc903",
         "title": "Home Alone",
         "appId": "YouTube",
         "poster":"data:image/png;base64, iVBsRw0KdGgoAAAANSUhE...",
      ...
      {
         "entryId": "a7eb554a-27bd-40e4-825f-cc7ac90fb6b2",
         "title": "I Don't Feel at Home in This World Anymore",
         "appId": "Netflix",
         "poster":"data:image/png;base64, iVBO9zQrdWqqtSDa...",
         "categories": ["Movies"],
      },
   ]
}
...
This operation retrieves content recommendations from the device.
The returned content must match the content displayed in the device's recommendations menu.

#### Request Topic

`dab/<device-id>/content/recommendations`
...
```typescript
interface ListRecommendationsResponse extends DabResponse {
   entries: ContentEntry[],
}
```
...
### Operation: Open content
*Operation model: Request / Response*

This operation opens the specified content and returns only after the application has started.

It mimics a user clicking on the content tile in search results or recommendations.

Note: it differs from the launch application with content id operation where content id is an application specific content id.


#### Request Topic

`dab/<device-id>/content/open`

#### Request Format

```typescript
interface OpenContentRequest extends DabRequest {
   entryId: string,
}
```
...
Parameter | Default|  Description
--- | --- | ---
entryId | N/A | The ID of the content.

#### Sample Request

```json
{
   "entryId": "homealone123"
}
```
...
```typescript
type OpenContentResponse = DabResponse;
```
...
{
   "status": 404,
   "message": "Content 'a7eb554a-27bd-40e4-825f-cc7ac90fb6b2' not found."
}
```
